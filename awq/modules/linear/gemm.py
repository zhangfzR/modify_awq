...
class AdaRoundQuantizer(nn.Module):
    ...
    def __init__(self, bit, scale, group_size, x):
        super().__init__()
        self.bit = bit
    ...
    def init_alpha(self, x):
        ...
        scale = self.scale
        scale = scale.repeat(self.group_size, 1)
        scale.requires_grad = True
        x_floor = torch.floor(x.t() / scale)
        rest = (x.t() / scale) - x_floor
    ...
    def forward(self):   
        round_vals = self.get_soft_targets()
        round_loss = (1 - ((round_vals - 0.5).abs() * 2).pow(2)).to(torch.float32)
        round_loss = round_loss.sum()
        return round_loss     
...
class WQLinear_GEMM(nn.Module):
    ...
    @classmethod
    def from_linear(
        cls, linear, w_bit, group_size, init_only=False, scales=None, zeros=None, use_adaround=False
    ):
        awq_linear = cls(
            w_bit,
            group_size,
        ...
        if use_adaround:
            adaround = AdaRoundQuantizer(w_bit, scales, group_size, linear.weight.data.clone())
            optimizer = torch.optim.Adam(adaround.get_train_params(), lr=0.1, eps=1)
            for i in range(10):
                loss = adaround()
                optimizer.zero_grad()
                loss.backward()
                print(f"Adaround alpha: {adaround.alpha}")
                print(f"Adaround alpha grad: {adaround.alpha.grad}")
                optimizer.step()
        ...
        for idx in range(awq_linear.in_features):
            intweight.append(
                torch.round(
                    # ((linear.weight.data[:, idx] + scale_zeros[idx // group_size]+adaround.get_soft_targets().t()[:, idx])
                    #  if use_adaround else
