...
from awq.utils.utils import get_best_device, FakeLinear,clear_memory
...
def apply_lwc(module):
    if isinstance(module, FakeLinear):
        ...
        if not hasattr(module.ori_layer, "zeros"):
            module.ori_layer.zeros = module.quant_module.zeros
        del module.quant_module
        module = module.ori_layer
    else:
        name_fakelayer = {name:layer for name, layer in module.named_modules() if isinstance(layer, FakeLinear)}
        for name, layer in name_fakelayer.items():
            if isinstance(layer, FakeLinear):
                ...
                if not hasattr(layer.ori_layer, "zeros"):
                    layer.ori_layer.zeros = layer.quant_module.zeros
                setattr(module, name, layer.ori_layer)
                del layer
        del name_fakelayer
    clear_memory()
    module.half().cpu()
